...
namespace Basta.GUI.Login.Game {
    ...
    public partial class GameWindow: Window {
        ...
        public void StartShowResults( Dictionary<string, Dictionary<GameCategory, Word>> results, Player winner ) {
            ...
            this.winner = winner;
            TimerToShow( ShowWinnerToLabels, null, (int) GameConfiguration.TimeToShowResults );
        }
        ...
        private void CallServiceToShowMainScreenBorder() {
            try {
                Autentication.GetInstance().GameServer.ShowMainScreenBorder( Room );
            } catch ( Exception ex ) {
                if ( ex is EndpointNotFoundException || ex is CommunicationException ) {
                    DropConnectionAlert.ShowDropConnectionAlert();
                    Close();
        ...
        private void CallServiceToShowGridGame() {
            try {
                Autentication.GetInstance().GameServer.ShowGridGameFromRoom( Room );
            } catch ( Exception ex ) {
                if ( ex is EndpointNotFoundException || ex is CommunicationException ) {
                    DropConnectionAlert.ShowDropConnectionAlert();
                    Close();
        ...
        private void ShowWinnerToLabels() {
            ...
            TimerToShow( CallServiceToStartNewRound, null, (int) GameConfiguration.TimeToEndRound );
        }
